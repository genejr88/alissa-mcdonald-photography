...
export default function BookingToken() {
  ...
  if (isError) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--paper)' }}>
      <p className="opacity-60 text-sm tracking-widest uppercase">Booking not found.</p>
    </div>
  );
      ...
      <div className="max-w-xl mx-auto px-6 pt-24 pb-24">
        ...
        >
          ...
          {status === 'PENDING' && (
            <div className="space-y-4">
              <p className="text-sm opacity-70 leading-relaxed">
                I&rsquo;ll review your request and confirm your spot soon — you&rsquo;ll get an
                email when it&rsquo;s official.
              </p>
              <button
                className="text-sm opacity-70 hover:opacity-80 transition-opacity"
                onClick={() => {
          ...
          {status === 'CONFIRMED' && (
            <div className="space-y-4">
              <p className="text-sm opacity-70 leading-relaxed">
                Your session is confirmed! Keep an eye on your email — if a contract needs your
                signature, you&rsquo;ll receive a secure signing link there.
              </p>
            </div>
          )}
          ...
          {status === 'CANCELLED' && (
            <div>
              <p className="text-sm opacity-70">This booking has been cancelled.</p>
...
function Row({ label, value }) {
  return (
    <div className="flex gap-4">
      <span className="text-xs tracking-widest uppercase opacity-60 w-28 shrink-0 pt-0.5" style={{ fontFamily: 'var(--font-mono, monospace)' }}>
        {label}
      </span>
